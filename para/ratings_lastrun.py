...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Sophie\\Documents\\BP\\template-ratings - Kopie\\para\\ratings_lastrun.py',
...
trials = data.TrialHandler(nReps=1, method='random', 
    ...
    trialList=data.importConditions('lists/img2.xlsx'),
...
for thisTrial in trials:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if valence.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > valence.tStartRefresh + 2-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if arousal1.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > arousal1.tStartRefresh + 2-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if val_slider.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > val_slider.tStartRefresh + 2-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if ar_slider.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > ar_slider.tStartRefresh + 2-frameTolerance:
                # keep track of stop time/frame for later
